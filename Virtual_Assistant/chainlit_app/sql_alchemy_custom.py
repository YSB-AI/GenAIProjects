...
class SQLAlchemyDataLayer(BaseDataLayer):
    ...
    async def force_thread_creation(self, function_calling_name : str, step_dict: "StepDict") :
        
        thread_id =  step_dict["threadId"]
        user_identifier =  cl.context.session.user.identifier
        thread_name =  step_dict["output"] 
            
        print(f"{function_calling_name} - thread '{thread_id}' from user '{user_identifier}' with name '{thread_name}' forced being created....")
        user_id =  cl.context.session.user.id
    ...
    async def create_step(self, step_dict: "StepDict"):
        ...
        #Create thread if it missing
        thread = await self.get_thread(step_dict["threadId"])
        if  thread is None:
            print("create_step() Force creating thread ...")
            await self.force_thread_creation("create_step()",step_dict)     
